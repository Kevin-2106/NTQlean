...
**默认只选孤儿缓存（聊天缓存清理主目标）**。有数据库有效记录的文件——filerecv 下载、
另存为等——默认**保留**，需要时手动勾选对应置信度。

孤儿（orphan）= 没有任何路径级有效引用的 nt_data 文件。判定规则：

- 只有数据库记录自身的路径成功解析到文件（`exact` / `strong`）才算"认领"；
  仅文件名匹配（`heuristic`，files_in_chat 约 40% 的行没有路径字段）**不认领**——
  这类多为过期的下载/另存为账目，曾把大量可清理文件挡在孤儿集之外；
- 孤儿再经 nt_msg 消息体 md5 扫描（需勾选 nt_msg 大库重建）确认：仍被聊天记录引用的
  予以排除（删了会影响聊天内容显示），引用为零的才是真正"删了不丢东西"的部分。
  未扫描 nt_msg 时 GUI/CLI 会明确警告"引用情况未知"。

- **类型**：图片 / 视频 / 文件 / 语音 / 其他
- **置信度**：`exact`（路径与大小均吻合）/ `strong`（路径吻合但大小不一致）/
  `heuristic`（仅文件名匹配，不作为有效引用）/ `orphan`（无有效引用，默认勾选）
