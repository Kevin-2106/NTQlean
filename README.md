...
**默认只清理「无主文件」**。有数据库有效记录的文件——filerecv 下载、
另存为等——默认**保留**，需要时手动勾选对应可信度。

引用可信度四级（结果与报告中显示中文名，CLI/JSON 保留英文值）：

| 显示名 | 英文值 | 含义 |
| --- | --- | --- |
| 记录吻合 | `exact` | 数据库记录的路径与大小都指向此文件 |
| 路径吻合 | `strong` | 路径指向此文件，但大小不一致 |
| 仅同名 | `heuristic` | 记录的原路径已失效/为空，仅文件名相同——**不算有效引用** |
| 无主文件 | `orphan` | 没有任何有效引用的磁盘文件——**清理主目标** |

无主文件的判定：只有路径级有效引用（记录吻合/路径吻合）才算"认领"；
仅同名匹配多为过期的下载/另存为账目（files_in_chat 约 40% 的行没有路径），
**不再**把文件挡在无主文件之外。无主文件再经 nt_msg 消息体 md5 扫描
（需勾选 nt_msg 大库重建）确认：仍被聊天记录引用的予以排除（删了会影响聊天显示），
引用为零的才是真正"删了不丢东西"的部分。未扫描 nt_msg 时 GUI/CLI 会明确警告
"引用情况未知"。**结果按文件去重**：同一文件只出现一次（无主文件行优先，
多条记录指向同一文件时只保留一条，字节数不再重复计）。

> **「仅同名」声明**：同名不代表记录真实指向此文件，更不代表文件仍被聊天引用。
> 勾选它只用于查看这些记录的来源会话，不应作为清理依据。

- **类型**：图片 / 视频 / 文件 / 语音 / 其他
- **引用可信度**：记录吻合 / 路径吻合 / 仅同名 / 无主文件（对应英文值
  `exact` / `strong` / `heuristic` / `orphan`；默认只勾无主文件）
...
- **按文件排除（保留名单）**：结果里右键 →「排除此文件（保留，不再选中）」，被排除的文件
  在任何筛选/预演/清理计划中都不再出现。名单持久保存在工作区 `excluded-files.txt`
  （一行一个路径，`#` 注释，可手改），跨会话生效；统计区可一键清空。
  CLI 等价：`--exclude-file <path>`（可重复）或 `--exclude-from <list.txt>`。
...
| 媒体索引 / 孤儿分析 / 群名提取 | ✅ 真实数据实测（孤儿集按"无有效路径引用"判定） |
| nt_msg 引用确认（二进制 + ASCII md5） | ✅ 真实数据实测；未扫描时明确警告 |
